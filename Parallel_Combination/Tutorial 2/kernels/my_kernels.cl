...
// TREATS THE IMAGE AS A 1D ARRAY WITHOUT DISTINGUISHING BETWEEN COLOURS
...
kernel void identity(global const uchar* A, global uchar* B) {
	...
	if(id == 0) {printf("work group size %d\n", get_local_size(0));}
...
kernel void filter_r(global const uchar* A, global uchar* B) {
	...
	// all other components (Green + Blue) should be set to 0.
...
kernel void identityND(global const uchar* A, global uchar* B) {
	...
	int c = get_global_id(2); //current colour channel (spectrum?)
