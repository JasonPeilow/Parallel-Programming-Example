...
//a very simple histogram implementation (Atomic)
kernel void hist_simple(global const int* A, global int* H, int nr_bins) { 
	int id = get_global_id(0);
	int lid = get_local_id(0);
	...
	//H[bin_index] += 1 //Note: Atomic operations stop race conditions, but serialise (slow) global memory access 
}
